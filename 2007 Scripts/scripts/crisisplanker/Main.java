...
public class Main extends PollingScript<ClientContext> implements PaintListener {
	...
	public void start() {
		...
		logPrice = price(log.getLogId());
		plankPrice = price(log.getPlankId());
		profitPerLog = plankPrice - logPrice;
	...
	public void repaint(Graphics g1) {
		...
		g.drawString("Profit: " + NumberFormat.getInstance().format(totalProfit), 363, 188);
		g.drawString("Profit/hr: " + NumberFormat.getInstance().format(getPerHour(totalProfit, getRuntime())), 363, 212);
		g.drawString(
	...
	public TilePath walkToGuy() {
	  ...
	  int price(int id) {
	        final Matcher matcher = Pattern.compile("\"average\":\"\\d+").matcher(text(id));
	        if (matcher.find()) {
	     ...
	     String text(int id) {
	        try {
	            final URL add = new URL("http://forums.zybez.net/runescape-2007-prices/api/item/" + id);
